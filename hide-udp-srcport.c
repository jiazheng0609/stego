...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    fprintf(stderr, "Usage: %s <input_pcap_file> <secret_file> <output_pcap_file>\n", argv[0]);
    return 1;
  }
  ...
  while ((packet = pcap_next(pcap, &header)) != NULL) {
	...
	if (hide_cnt < sec_file_size) {
        ...
        tmp = (sec_buffer[hide_cnt+1] << 8 ) | sec_buffer[hide_cnt];
		udp->sport = htons(tmp);
		hide_cnt += 2;
	}
  ...
  if (hide_cnt < sec_file_size) {
    printf("Error! Covert packet not enough to hide all messeges.\n");
  }
  else {
	printf("Success!\n");
  }
