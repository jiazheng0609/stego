...
int main(int argc, char *argv[])
{
		...
		if (rtp->version == 2 && rtp->pt == 8) {	/* We only deal with RTP v2 and G.711 */
			...
			for (int i = 0; i < last_byte_pos; i++) {
				if (hide_cnt < sec_file_size) {
					...
					printf("byte %d %x bit %d\n", i,
					       payload[i], bit_in_char);
				} else if (hide_cnt >= sec_file_size
					   && (hide_cnt < sec_file_size + 7)) {
					/* 8 consecutive magic number to represent end of secret file */
					payload[i] = (payload[i] & 0xfe) & 0;
					hide_cnt++;
					printf("byte %d %x bit %d\n", i,
					       payload[i], bit_in_char);
				}
	...
	if (hide_cnt < sec_file_size) {
		printf
		    ("Error! Covert RTP packet not enough to hide all messeges.\n");
	} else {
		printf("Success!\n");
	}
